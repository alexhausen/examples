...
// initial value: 0
// final xor value: 0
// compare to result to http://www.sunshine2k.de/coding/javascript/crc/crc_js.html
...
uint16_t crc16(const uint8_t* input, size_t size) {
  uint16_t crc = 0;
  for (size_t i = 0; i < size; ++i) {
    uint8_t idx = ((crc >> 8) ^ input[i]) & 0xFF;
...
int main(void) {
  ...
  uint8_t test0[] = { 0 };
  size_t test0_size = sizeof(test0)/sizeof(uint8_t);
  uint16_t test0_crc = crc16(test0, test0_size);
  assert(0 == test0_crc);

  uint8_t test1[] = { 1 };
  size_t test1_size = sizeof(test1)/sizeof(uint8_t);
  uint16_t test1_crc = crc16(test1, test1_size);
  assert(0xd175 == test1_crc);

  uint8_t test2[] = { 0x47, 1, 5, 0, 0, 0, 0, 0, 1 };
  size_t test2_size = sizeof(test2)/sizeof(uint8_t);
  uint16_t test2_crc = crc16(test2, test2_size);
  assert(0xfcbe == test2_crc);

  uint8_t test3[] = { 'a', 'b', 'c' };
  size_t test3_size = sizeof(test3)/sizeof(uint8_t);
  uint16_t test3_crc = crc16(test3, test3_size);
  assert(0x713c == test3_crc);
