#include <stdio.h>
#include <stdint.h>
#include <assert.h>
#include <string.h>

// poly 0x97

uint8_t crc_table[] = {

};

uint8_t crc8_no_table(const uint8_t data[], size_t sz) {
  uint8_t crc = 0;
  for (size_t i = 0; i < sz; i++) {
    crc ^= data[i];
    for (size_t j = 0; j < 8; j++) {
      if ((crc & 0x80) != 0) {
        crc = (crc << 1) ^ 0x97;
      } else {
        crc = crc << 1;
      }
    }
  }
  return crc;
}

uint8_t crc8_table(const uint8_t data[], size_t sz) {
  uint8_t crc = 0;
  for (size_t i = 0; i < sz; i++) {
    crc = crc_table[crc ^ data[i]];
  }
  return crc;
}

typedef uint8_t (*crc_fn)(const uint8_t*, size_t);

int main(int argc, char** argv) {

  crc_fn crc8 = crc8_no_table;

  if (argc == 1) {
    // test
    uint8_t test0[] = { 0x0 };
    uint8_t crc = crc8(test0, 1);
    assert(crc == 0x0u);

    uint8_t test1[] = { 0xFF };
    crc = crc8(test1, 1);
    assert(crc == 0x3Cu);

    uint8_t test2[] = { 'a', 'b', 'c' };
    crc = crc8(test2, 3);
    assert(crc == 0x60u);

    puts("ok");
  } else if (argc == 2) {
    // user input (ASCII)
    uint8_t crc = crc8((uint8_t*)argv[1], strlen(argv[1]));
    printf("0x%02X\n", crc);
  } else {
    // usage
    printf("crc8.exe [<input>]\n");
  }

  return 0;
}
