...
func (app *application) authenticate(w http.ResponseWriter, r *http.Request) {
	...
	// send token to user
	_ = app.writeJSON(w, http.StatusOK, tokenPair)
}
...
func (app *application) refresh(w http.ResponseWriter, r *http.Request) {
	...
	now := time.Now()
	time_diff := time.Unix(claims.ExpiresAt.Unix(), 0).Sub(now)
	if time_diff > 30*time.Second {
		app.errorJSON(w, errors.New("refresh token does not need renew yet"), http.StatusTooEarly)
		return
	}
