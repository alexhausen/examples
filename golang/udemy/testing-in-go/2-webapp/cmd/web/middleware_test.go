...
func Test_application_ipFromContext(t *testing.T) {
	...
	ctx = context.WithValue(ctx, contextUserKey, "198.1.2.3")
	ip := app.ipFromContext(ctx)
	if ip != "198.1.2.3" {
		t.Errorf("wrong IP from context, %s", ip)
	}
}
...
func Test_app_auth(t *testing.T) {
	...
	for _, e := range tests {
		...
		if e.isAuth && rr.Code != http.StatusOK {
			t.Errorf("%s: expected OK, got %d", e.name, rr.Code)
		}
		if !e.isAuth && rr.Code != http.StatusTemporaryRedirect {
			t.Errorf("%s: expected status 307, got %d", e.name, rr.Code)
		}
	}
}
