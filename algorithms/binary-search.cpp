...
template <class T>
auto bin_search_recursive_impl(const T &value, const std::vector<T> &vec,
                               unsigned lower, unsigned upper) -> int {
...
auto binary_search_iteractive2(const T &value, const std::vector<T> &vec) {
  auto it = std::lower_bound(vec.cbegin(), vec.cend(), value);
  return *it == value ? it : vec.cend();
}

// branchless binary search
template <class T, class Iter>
auto binary_search_branchless_stl(const T &value, Iter lower, Iter upper) {
  Iter range[] = {lower, upper};
  while (range[0] < range[1]) {
    auto mid = std::next(range[0], std::distance(range[0], range[1]) / 2);
    bool b = *mid >= value;
    range[b] = mid + !b;
  }
  return range[1];
...
template <class T>
auto binary_search_branchless(const T &value, const std::vector<T> &vec) {
  return binary_search_branchless_stl(value, vec.cbegin(), vec.cend());
}
...
int main() {
  ...
  assert(99 == *it7);
  auto it8 = binary_search_iteractive2(100, vec1);
  assert(it8 == vec1.cend());

  auto it9 = binary_search_branchless(1, vec1);
  assert(it9 != vec1.cend());
  assert(1 == *it9);
  auto it10 = binary_search_branchless(330, vec1);
  assert(it10 != vec1.cend());
  assert(330 == *it10);
  auto it11 = binary_search_branchless(11, vec1);
  assert(it1 != vec1.cend());
  assert(11 == *it11);
  auto it12 = binary_search_iteractive2(100, vec1);
  assert(it12 == vec1.cend());
